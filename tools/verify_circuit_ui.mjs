...
class El {
  ...
  getBoundingClientRect() { return { left: 0, top: 0, width: 900, height: 400, right: 900, bottom: 400 }; }
...
/* ---------------------------------------------------------------- 7. disposal */
section('dispose', () => {
  const before = liveWindowListeners;
  const h = mount({ model: { parts: [], wires: [] } });
  h.cv.focus();
  h.tool('R');
  ...
  const cv = h.cv;
  h.handle.dispose();
  if (liveWindowListeners !== before) {
    note('dispose', 'dispose() left ' + (liveWindowListeners - before) + ' window listener(s) behind');
  }
  ...
  if (liveWindowListeners !== before) note('dispose', 'a second dispose() unbalanced the window listeners');
});
