...
/* A renderer that owns main.innerHTML destroys whatever was focused inside it, and the
   browser drops focus to <body> — so a keyboard user who presses a button that repaints
   the screen is silently returned to the top of the document. Put focus back on the
   control that did the thing, which is where it would have stayed had the DOM survived. */
...
/* null until the first answer, then true while progress is being stored and false while
   it is not. The transition is what gets announced; the word in the topbar is not. */
let saveHealth = null;
function setSaveState(s, bad) {
  const e = $('#save-state');
  if (e) {
    /* The word moved into a child so the element itself can be the live region without
       the word being what it reads. Falls back to the box for any shell that predates
       the split. */
    const txt = $('#save-state-txt') || e;
    txt.textContent = s;
  ...
  /* The visible word changes on every save, and a session is hundreds of saves. Piping
     that into a live region would read "Saving, Saved, Saving, Saved" over the top of
     whatever the learner was actually listening to, which is how a live region gets
     switched off. What is worth announcing is the *health* changing: the moment progress
     stops being stored, and the moment it starts again. Nothing in between. */
  if (s === 'Saving…') return;
  const healthy = !bad;
  if (healthy === saveHealth) return;
  const first = saveHealth === null;
  saveHealth = healthy;
  const say = $('#save-state-say');
  if (!say) return;
  say.textContent = healthy
    ? (first ? '' : 'Progress is being saved again.')
    : 'Progress is not being saved in this browser. Open Profile to export it before you close this tab.';
}
...
/* Store.status()'s own comment says it is "called before the first save so the UI can
   warn straight away rather than after the learner has already earned progress they are
   about to lose". Nothing ever called it. setSaveState ran only from inside saveNow, so
   the indicator was blank from boot until the first write, and in a private window the
   first thing it ever said was "Not saved", after the work was already at risk. Asked at
   boot instead. Silent when the answer is good: a store that works is not news, and the
   strip is 44px wide. */
function paintSaveState() {
  const st = Store.status();
  if (st.persistent) { saveHealth = true; return; }
  setSaveState('Not saved', true);
  warnNoStorage();
}
/* saveSoon is debounced by 900 ms, which is the right cadence for a slider being dragged
   and the wrong one for a tab being closed. `saveDirty` is set when the intent to save is
   formed rather than when the timer fires, so the unload path knows something is owed
   even though nothing has been written yet. */
let saveDirty = false;
const saveDebounced = debounce(function () { saveNow(); }, 900);
function saveSoon() { saveDirty = true; saveDebounced(); }
function saveNow() {
  saveDirty = false;
  setSaveState('Saving\u2026');
...
/* The last 900 ms of every session were never written. Finish a unit and close the tab
   inside the debounce window and the completion, its XP and the day's activity were
   simply gone; on a phone the tab is backgrounded and killed with no close at all.
   desk.js has flushed the *calculator* on pagehide since cycle 12. The file that carries
   every completed unit, every quiz score and every saved file did not.
   This cannot go through saveChain: that queues a microtask, and an unloading document is
   under no obligation to run one. Store.saveSync writes on the spot. */
function flushSave() {
  if (!saveDirty) return false;
  saveDirty = false;
  P.updatedAt = Date.now();
  const ok = Store.saveSync(P);
  setSaveState(ok ? 'Saved' : 'Not saved', !ok);
  if (!ok) warnNoStorage();
  return ok;
}
if (typeof window !== 'undefined' && window.addEventListener) {
  /* pagehide rather than beforeunload: beforeunload never fires on iOS, and it also asks
     the browser for a dialog this has no business showing. visibilitychange is the one
     that catches a phone backgrounding the tab, which is the common case and the one that
     sends no unload event at all. */
  ...
  $('#app').innerHTML =
    ...
    '<div class="frame">' +
      '<header class="topbar">' +
        ...
        /* The only channel in the app that says whether progress is being kept, and it
           was a bare span: no role, no name, no live region. It went to "Not saved" and
           a screen reader said nothing at all. The word is a child so the box can carry
           the announcement without reading the word — see setSaveState. */
        '<span class="save-state" id="save-state">' +
          '<span class="ss-txt" id="save-state-txt" aria-hidden="true"></span>' +
          '<span class="vh" id="save-state-say" role="status" aria-live="polite"></span>' +
        '</span>' +
      '</header>' +
...
function adopt(progress) {
  ...
  adopting = true;
  try {
    /* Three doors led into P — boot, adopt and import — and each trusted the document
       it was handed to a different degree. That disagreement was the defect, not any
       one of the three: the loosest door was the one opening onto a file off a disk.
       One rule now, applied at all three. */
...
/* ---------- taking a progress document from outside ----------
   The import path took whatever the file said and assigned it straight into P. What
   that actually allowed, driven rather than reasoned about:

     "xp": "9999"      reached the topbar as a level-67 badge — a string has its own
                       toLocaleString, so nothing threw and nothing looked wrong;
     "xp": {"a":1}     reached it as "[object Object]", at level NaN;
     "activity": [..]  passed `typeof === 'object'` (an array is one), and then every
                       lookup was undefined, so the streak read 0 for ever;
     "completed"       counted `{"B": false}` as a finished unit in the toast;
     "__proto__"       reached Object.assign, whose writes go through [[Set]] — so an
                       inherited setter ran and the prototype of P itself was replaced.

   None of these threw. All of them were then saved, and pushed: the server settles xp
   with `Math.max(Number(a.xp) || 0, ...)`, so a non-numeric xp does not merely display
   wrongly, it silently zeroes the XP the account already held.

   The rule below is the one adopt() has always followed on the sync path — collections
   are objects of the shape we declare, scalars are coerced, and XP is derived from
   `completed` rather than believed. The two doors into P disagreed, and the trusting one
   was the door that opens onto a file off a disk. */
/* Inside the function rather than beside it on purpose: boot() and adopt() both call
   sanitiseProgress, a function declaration hoists and a `const` does not, and cycle 6
   already shipped one temporal-dead-zone read in this codebase from exactly that
   asymmetry. Nothing here is hot enough for the allocation to matter. */
function plainObject(v) {
  /* Array.isArray is the half `typeof` cannot do. `__proto__` is skipped rather than
     copied because copying it is the pollution. */
  const out = {};
...
function sanitiseProgress(inc) {
  const SLOTS = ['completed', 'quiz', 'code', 'derive', 'build', 'blanks',
                 'numeric', 'match', 'tune', 'activity'];
  const src = plainObject(inc);
  const out = { completed: {}, quiz: {}, code: {}, derive: {}, build: {}, blanks: {},
                numeric: {}, match: {}, tune: {}, activity: {},
                xp: 0, last: null, playground: null, name: '', railHidden: false };
  for (const k of SLOTS) out[k] = plainObject(src[k]);
  /* completed is the one collection whose values are read as truth rather than as data,
     so a falsy entry is not a completion and must not be counted as one */
  const done = {};
  for (const k of Object.keys(out.completed)) if (out.completed[k]) done[k] = true;
  out.completed = done;
  /* a count of units finished that day; a string that happens to parse is still not one */
  const act = {};
  for (const k of Object.keys(out.activity)) {
    const n = Number(out.activity[k]);
    if (isFinite(n) && n > 0) act[k] = Math.floor(n);
  }
  out.activity = act;
  ...
  if (isFinite(seed) && seed >= 0) out.seed = Math.floor(seed);
  /* A floor only. recomputeXp() replaces it with the exact figure the moment the catalog
     can value every completed unit, and holds this number when a payload is missing —
     which is the same contract adopt() has. */
  const xp = Number(src.xp);
  out.xp = isFinite(xp) && xp > 0 ? Math.floor(xp) : 0;
  const at = Number(src.updatedAt);
  if (isFinite(at) && at > 0) out.updatedAt = Math.floor(at);
  /* The reset tombstone has to survive every door into P, or a reload re-offers the
     document the learner erased and the merge lets it back in */
  const cl = Number(src.clearedAt);
  if (isFinite(cl) && cl > 0) out.clearedAt = Math.floor(cl);
  return out;
}
...
function importProgress(file) {
  ...
  fr.onload = function () {
    ...
    P = sanitiseProgress(inc);
    /* An import replaces — that is what it has always done locally, and what "Restored
       N completed units" claims. Remotely it merged, so importing a one-unit export onto
       a three-unit account left all three and said 1. `clearedAt` is what makes the two
       agree; without it the sentence below is false about the account within seconds. */
...
function resetProgress() {
  ...
  /* Without this the card's "this cannot be undone" was the one thing here that was not
     ...
     later — locally as well, because syncNow adopts what comes back and writes it to the
     store. The toast stayed on screen saying "Progress cleared" over the top of it. */
  P.clearedAt = Date.now();
  updateXp();
  renderRail();
  saveNow();
  go({ view: 'profile' });
  /* go() replaces main.innerHTML, so the button that was just pressed no longer exists
     and focus falls to <body> — a keyboard user is dropped at the top of the document
     with nothing said. Put it back on the control that did the thing. */
  focusAfterRender('#prof-reset');
...
function accountCardHtml() {
  ...
  return '<div class="pcard">' +
    ...
    '<div class="pcard-acts">' +
      ...
      /* "Wrong email or password" was written into a bare span. A screen-reader user
         pressed Sign in, heard nothing, and had no way to find out why they were still
         on the form. It is the same span as the reset note and it needed the same fix. */
      '<span class="prof-warn" id="acc-msg" role="status" aria-live="polite"></span>' +
    '</div>' +
  '</div>';
}
...
function wireAccountCard(main) {
  ...
  if (!email) return;
  /* `disabled` on the button that was just pressed takes focus with it — the browser
     blurs a disabled element and focus falls to <body>, so a keyboard user pressing Sign
     in was thrown to the top of the page for the length of the request and then had to
     find their way back to read the failure. aria-disabled says the same thing to
     assistive technology without moving anything, and `attempting` is what actually
     stops the second press. */
  let attempting = false;
  const busy = function (on) {
    attempting = on;
    ['#acc-in', '#acc-new'].forEach(function (sel) {
      const b = $(sel, main);
      if (b) { b.setAttribute('aria-disabled', on ? 'true' : 'false'); b.classList.toggle('is-busy', on); }
    });
  };
...
function renderProfile(main) {
  ...
  main.innerHTML = '<div class="page">' +
    ...
    '<div class="pcard">' +
      ...
      '<div class="pcard-acts">' +
        ...
        /* The note used to be `hidden` with its sentence already inside it, and arming
           the button un-hid it. Revealing text that a live region already held announces
           nothing in most screen readers — the region has to be present and empty, and
           the text has to arrive. So the sentence moved into the handler.
           assertive, not polite: the very next thing this learner does is press the
           button again, and a queued announcement arrives after the erase. */
        '<span class="prof-warn" id="prof-reset-note" role="status" aria-live="assertive"></span>' +
      '</div>' +
    '</div>' +
  '</div>';
  ...
  /* The two-press confirmation was invisible to the people most likely to press twice.
     Arming it un-hid a span and changed nothing else: the button's accessible name stayed
     "Reset progress" in both states, so a screen-reader user pressed it, heard nothing at
     all, pressed it again because nothing had happened — and that second press erased
     everything. A confirmation that is silent is not a confirmation; it is a delay.
     Three changes: the sentence is announced, the button says which press this is, and
     the window is long enough to read it in. The visible label changes with the
     accessible name rather than instead of it, because a name that does not contain the
     visible label is its own defect (WCAG 2.5.3). */
  let armed = false, disarmTimer = null;
  const resetBtn = $('#prof-reset', main), resetNote = $('#prof-reset-note', main);
  const ARMED_LABEL = 'Confirm — erase all progress';
  const disarm = function () {
    clearTimeout(disarmTimer);
    disarmTimer = null;
    armed = false;
    if (resetBtn) resetBtn.textContent = 'Reset progress';
    /* cleared rather than re-announced: disarming fails safe, and a live region that
       reports both edges of a four-second timer is noise on top of a destructive act */
  ...
  resetBtn.addEventListener('click', function () {
    if (!armed) {
      ...
      resetNote.textContent = 'Press again to erase every completed unit, your XP and your saved code. This cannot be undone.';
      /* 4000 ms was not long enough to hear the sentence, find the button again and
         press it. Re-armed from scratch each time rather than stacking timers. */
      clearTimeout(disarmTimer);
      disarmTimer = setTimeout(disarm, 12000);
      return;
    }
    disarm();
    resetProgress();
...
async function boot() {
  ...
  /* Store.load already refuses anything that will not parse. What it cannot refuse is a
     ...
     one, or one written by a build that held a field differently. The XP figure is the
     one exception to the rule and is left as stored rather than recomputed here: this
     runs before loadDegreeChunks, so the catalog cannot yet value a course unit. */
  if (saved && typeof saved === 'object') P = sanitiseProgress(saved);
  renderShell();
  /* the indicator has been blank from boot until the first write for as long as it has
