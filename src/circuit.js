...
const Symbols = (function () {
  ...
  define('V', 'Voltage source', function (c) {
    ...
    c.moveTo(-60, 0); c.lineTo(-30, 0);
    c.moveTo(30, 0); c.lineTo(60, 0);
    /* + on the RIGHT. It was on the left here and on the right in the editor, which
       is two answers to which end is positive with one of them wrong — in the one
       subject where that is the whole question. Outside the circle and above the
       lead, so it is beside the terminal it names rather than crowding the rim. */
  ...
  define('I', 'Current source', function (c) {
    ...
    c.moveTo(-60, 0); c.lineTo(-30, 0);
    c.moveTo(30, 0); c.lineTo(60, 0);
    /* along the leads, not across them — see the editor's copy */
...
function createCircuit(root, opts) {
  ...
  let codeFull = false;        /* the MCU sketch is filling the screen */
  ...
  function drawPart(p, colour, pal) {
    ...
    if (p.kind === 'R') {
      ...
      /* V and I share a circle. What is in it says which: an arrow along the leads
         for a current source, and nothing at all for a voltage source, whose + and −
         are drawn after the restore below, upright. */
      ctx.moveTo(-L, 0); ctx.lineTo(-11, 0);
      ctx.moveTo(11, 0); ctx.lineTo(L, 0);
      ctx.stroke();
      ctx.beginPath();
      ...
      if (p.kind === 'I') {
        ctx.beginPath();
        /* THROUGH the source, along the leads, pointing at the + terminal — which is
           the way current runs inside a source. It used to point across the device,
           at right angles to both its pins, which is a direction nothing flows in.
           An arrow may stay inside the rotation: an arrow turned is still an arrow. */
      ...
      /* A voltage source's + and − are drawn after the restore, below — a minus sign
         turned a quarter is a vertical tick. */
    }
    ctx.restore();

    /* Letters and signs go on outside the rotation, or a vertical meter reads
       sideways and a vertical source's minus reads as a lead. Where they SIT still
       follows the turn, because they name the terminals and the terminals move; only
       what they look like is held upright.

       Screen direction of the + terminal, by quarter turns: right, up, left, down.
       That is plusFirst's convention seen from outside — the RIGHT pin flat, the TOP
       pin rotated — and the two must not be allowed to disagree, because the sign a
       learner reads off the drawing is the sign they will put in the sum. The marks
       ride above a lying-down source and to the left of a standing one, so they are
       never on top of the value label, which centres itself above the body. */
    if (p.kind === 'V') {
      const r = turnsOf(p);
  ...
  function paintPart() {
    const p = selOne();
    /* The sketch box belongs to one part. Selecting anything else takes it off the
       screen rather than leaving an overlay for a part nobody is looking at.
       The class comes off the live box BEFORE syncExpanded runs, because syncExpanded
       reads the DOM and the panel below has not been rebuilt yet. Without that the
       overlay went away and the page stayed scroll-locked behind nothing. */
  ...
  function paintMcu(p) {
    ...
    partPanel.innerHTML = '<h4>Microcontroller ' + esc2(refOf(p)) + '</h4>' +
      (gone
        ...
        : '') +
      /* The sketch, its caption, its diagnosis and its console are one block, because
         filling the screen with the sketch and leaving the error it produced behind in
         a 250px column would be filling the screen with half of the thing.

         The caption names the box through aria-labelledby rather than by wrapping it
         in a <label>. It was a <label> — put there because a bare <span> is a legend
         for the eye and nothing at all for the accessibility tree — and the name is
         still a real one; what changed is that the caption row now also holds a
         button, and a button inside a label is a click the label steals. */
    ...
    /* Filling the screen with the sketch. The flag is module-level and the class is
       ...
       model changes and a state kept only in the DOM would be thrown away by the
       first edit. */
    ...
    partPanel.querySelector('[data-codefull]').addEventListener('click', function () {
      ...
      /* Focus follows the box, or expanding it would take the caret out of the thing
         it just made bigger. */
    ...
    /* Escape leaves the sketch. It cannot reach the canvas's own Escape from in here —
       a textarea swallows the key on its way past — so it is handled where it lands. */
  ...
  /* The page is scroll-locked, and .frame and .page stop being stacking contexts,
     while ANYTHING is filling the screen — the editor, the sketch box, or one of each
     in two editors on one page. The DOM is asked rather than a count kept, so a
     dispose or a repaint cannot leave the body locked with nothing over it. */
  ...
  function setFull(on) {
    ...
    /* .page carries an entry animation, and an element with an animation that can
       ...
       887px tall inside a 1320px window. The body class turns that animation off
       while anything is expanded, stops the page behind from scrolling, and drops
       .frame's z-index so the icon rail is not left sitting on top of the editor. */
    syncExpanded();
  ...
  return {
    ...
    dispose: function () {
      ...
      codeFull = false;
      root.innerHTML = '';
      /* After the DOM goes, not before: syncExpanded reads it. A lesson navigated away
         from while something was expanded would otherwise leave the page scroll-locked
