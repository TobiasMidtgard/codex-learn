...
import numpy as np
_a = amplitude(integrate(0.05, [1.9, 0.0], 0.002, 20000))
assert abs(_a - 2.0) < 0.01, \
    f"as mu goes to zero the averaging prediction a = 2 becomes exact; at mu = 0.05 the amplitude should be within a per cent of 2, got {_a:.6f}"
'''},
                ],
            },
        },
...
''',
                "notice": [
                    ...
                    "Now lift $\\varphi$ off zero. The crossing disappears and the trajectories settle into the band between the two faint lines. That band is the price: the state no longer reaches the surface, only its neighbourhood, and the steady error is proportional to $\\varphi$.",
                ],
            },
            "derive": {
...
''',
                "steps": [
                    ...
                    {
                        "prompt": "The boundary layer replaces $\\mathrm{sgn}(s)$ by $s/\\varphi$ while $|s| < \\varphi$, so inside the layer the law becomes $\\dot{s} = -\\frac{\\eta}{\\varphi}s$. Write the time constant of $s$ there.",
                        "answer": "\\frac{\\phi}{\\eta}",
                        "placeholder": "\\frac{\\varphi}{\\eta}",
                        "hint": "For $\\dot{s} = -\\alpha s$ the time constant is $1/\\alpha$. Here $\\alpha = \\eta/\\varphi$.",
                        "deconstruct": [
                            "Inside the layer the dynamics are linear: $\\dot{s} = -(\\eta/\\varphi)s$.",
                            "The time constant is the reciprocal of the rate, so $\\varphi/\\eta$.",
                        ],
                    },
                ],
                "closing": r'''
...
which for a disturbance bounded by $D$ and a switching gain $k$ is $|s| \to D\varphi/k$.

Take $\varphi$ to zero and the exactness returns along with the chatter. There is no
setting at which you get both, and choosing $\varphi$ is choosing how much error to
...
''',
        "deliverables": [
            ...
            "`sat`, `surface` and `control` implementing $u = -\\lambda x_2 - (\\text{gain\\_bound}(x) + \\eta)\\,\\mathrm{sat}(s/\\varphi)$.",
            "`simulate(x0, lam, eta, phi, dt, steps, d_amp, b)` closing the loop over `plant.step` and returning `(xs, ss, us)`.",
            "A comment at the top of `main.py` naming the $\\lambda$, $\\eta$ and $\\varphi$ you chose, and what each one bought.",
        ],
        "constraints": [
