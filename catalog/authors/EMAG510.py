...
''',
                "notice": [
                    ...
                    "Set $R = 100$ and $X = 0$. The load sits on the real axis at $\\Gamma = +1/3$. Take the line length to 0.25 λ and the marker travels exactly half way round the circle to $\\Gamma = -1/3$, which reads as 25 Ω. A quarter wave turned 100 Ω into $50^2/100$.",
...
'''},
        ...
        # ---- M3 -----------------------------------------------------------
        {
            ...
            "concepts": [
                ...
                "TE$_{10}$ has the lowest cutoff of all, and the usable band runs from it to the next mode up — normally TE$_{20}$ at twice the frequency.",
            ],
            "sandbox": {
...
''',
                "notice": [
                    "Start where it opens, $R = 2$, $X = 200$. The marker sits almost on the rim and the readout gives $|\\Gamma| = 0.995$, a VSWR of about 425:1, and a return loss under 0.05 dB. Practically everything sent in comes straight back — that is a guide driven below its cutoff.",
...
''',
                ...
                "closing": r'''
Notice that $\beta < \omega/c$ always: the wave crawls along the axis more slowly than
light, because part of its wavenumber is spent bouncing across the guide. The next
module makes that statement quantitative, and finds that the *phase* pattern does the
opposite.
''',
            },
            "lab": {
...
'''},
                    {"name": "propagation is slower along the axis than in free space", "code": r'''
_fc = 6.5571403762e9
_b = beta(10e9, _fc)
...
'''},
        ...
        # ---- M4 -----------------------------------------------------------
        {
            ...
            "concepts": [
                ...
                "Phase velocity $\\omega/\\beta$ exceeds $c$ near cutoff, carries no information, and violates nothing.",
...
''',
                "notice": [
                    "With $\\zeta = 0.15$ the phase goes from about $-11°$ at $\\omega_n/2$ to about $-169°$ at $2\\omega_n$ — almost the whole 180° inside a factor of four in frequency. Set $\\zeta = 1.5$ and over exactly the same span it moves only from about $-63°$ to about $-117°$. Same total phase shift, utterly different slope, and it is the slope that delays things.",
                    "Look at the magnitude plot far above the corner. With $\\omega_n = 20$ it reads about $-40$ dB at $\\omega = 200$ and about $-80$ dB at $\\omega = 2000$: 40 dB per decade, which is the two-pole roll-off. A guide below cutoff falls off faster still, because its decay is exponential in distance rather than a power of frequency.",
