...
''',
                "notice": [
                    ...
                    "Set $a_{11} = a_{22} = 0$, $a_{12} = 1$, $a_{21} = -1$. Every curve now wraps round the dot instead of running into it or out of it, closing on itself exactly, and the readout calls the pattern a *centre*. No electrostatic field can circulate like that. A magnetic field always does, and module 4 is about why.",
                    "If you want to see how much of that closure is the field and how much is the drawing, put the integrator slider on forward Euler. The same rings open into outward spirals, because a tangent step lands just outside the curve it is following every single time and the error compounds. Nothing in those four coefficients adds energy; the creep is the picture's, and EE131 module 9 measures exactly this error.",
                ],
            },
            "derive": {
...
''',
                "notice": [
                    "As it opens, the readout says trace = 0.00, det = 1.00 and calls the pattern a *centre*. Every trajectory closes on itself at a fixed distance from the middle: the direction of the velocity goes round and round, the speed never moves. That is the whole content of ‘a magnetic force does no work’, drawn.",
                    "Take the integrator slider off RK4 and onto forward Euler, and watch the closure break: the rings open out, so the drawn speed climbs even though no work is being done on anything. That is the sketching code, not the physics — but it is worth seeing once, because a simulation that quietly manufactures the very quantity you are trying to conserve is the most expensive kind of wrong, and module 1 met the same artefact.",
