...
''',
        ...
        # ---- M10 ----------------------------------------------------------
        {
            ...
            "read": [
                {
                    "title": "The same three components, and two filters that share their poles",
                    "minutes": 15,
                    "body": r'''
Three components sit on the bench: a 0.1 H inductor, a 2.5 µF capacitor and a 100 Ω
resistor. Wire them into a loop with a signal generator and probe across the capacitor,
and the swept response reaches exactly 2.000 at 318 Hz — its true peak, 2.066, sits a
little below that at 298 Hz — and then falls away at 40 dB per decade: a low-pass with a
resonant bump on it. Now unsolder nothing except the probe, and swap the
capacitor and the resistor round so the probe sits across the resistor instead. The same
three components, the same three values, and the curve is now zero at DC, exactly 1.000
at 318 Hz, and falling at 20 dB per decade on *both* sides. Two filters that could hardly
be more different in what they are for.

Then stop sweeping and put a step in. Both circuits ring at 308 Hz. Both envelopes decay
with a time constant of 2 ms. Both have finished inside about 9 ms. The transients are
indistinguishable from one another; the frequency responses are not remotely alike.

Something about these two circuits is shared and something is not, and the split is not
where a first look would put it. This module is the explanation, and it is one line of
linear algebra.

## One row per node, with $s$ in it

Module 6 wrote Kirchhoff's current law once per node and read a matrix off the result.
Nothing in that argument was about resistors. KCL says the currents leaving a node add to
whatever is injected into it, at every instant; the Laplace transform is linear, so
transforming that equation term by term gives the same statement about $I(s)$ and
$V(s)$. The law survives intact.

What does change is the element law. A capacitor obeys $i = C\,\mathrm{d}v/\mathrm{d}t$,
and the differentiation rule of module 1 turns that into
$I(s) = sC\,V(s) - Cv(0)$ — the initial-condition term module 4 builds whole circuits
around. With no initial charge the second term is absent and the capacitor reads

$$I(s) = sC\,V(s),$$

which has exactly the shape of Ohm's law with $sC$ where the conductance was. An
inductor obeys $v = L\,\mathrm{d}i/\mathrm{d}t$, so $V(s) = sL\,I(s)$ and
$I(s) = V(s)/(sL)$. Each element still says *current equals something times voltage
difference*; the something is a function of $s$ rather than a number. That something is
the **admittance**:

$$Y_R = \frac{1}{R}, \qquad Y_C = sC, \qquad Y_L = \frac{1}{sL}.$$

So the whole of module 6 carries over unaltered. The current leaving node $a$ through an
element to node $b$ is $y_{ab}(V_a - V_b)$; summing over the neighbours and gathering the
unknowns gives one row per node; and the matrix is assembled by the same stamping rule —
add each element's admittance to both diagonal entries, subtract it from both
off-diagonal entries, and give ground no row and no column. The result is

$$Y(s)\,V(s) = I(s),$$

with $Y$ still symmetric, because an inductor conducts the same in both directions for
the same reason a resistor does. The only difference from module 6 is that the entries
are rational functions of $s$ instead of numbers in siemens.

## Cramer's rule, and the denominator nobody gets to choose

Solve $Y(s)V(s) = I(s)$ by Cramer's rule and every node voltage comes out as

$$V_k(s) = \frac{\det Y_k(s)}{\det Y(s)},$$

where $Y_k$ is $Y$ with column $k$ replaced by the source vector. Look at what that says
before computing anything with it. The numerator depends on $k$ — on which node you
decided to call the output — and on where the sources are. The denominator does not
depend on either. It is one function of $s$, built from the components and how they are
wired, and it sits under every node voltage in the network at once.

The poles are the roots of that denominator. So the poles are a property of the circuit,
and asking where the poles of the voltage across the capacitor are is a slightly confused
question: the network has poles, and the capacitor node has one of the numerators.

Clearing the fractions makes this concrete. Every $1/(sL)$ drags a $1/s$ into the
determinant, so $\det Y(s)$ arrives as a ratio; multiply it by whatever power of $s$
removes the denominators and an ordinary polynomial is left, whose roots are the natural
frequencies. Its degree counts the independent energy-storing components, because each
capacitor contributes one factor of $s$ through $sC$ and each inductor one through
$1/(sL)$. A network of nothing but resistors has degree zero — no poles, no transient,
an answer that appears the instant the input does.

## Worked: the same three parts, wired two ways

Number the nodes. In both circuits the source holds node 1 at $V_{in}$, so it is known
and drops out of the unknowns; nodes 2 and 3 are what is left, and $Y$ is $2\times2$.

The low-pass has the inductor from 1 to 2, the resistor from 2 to 3, and the capacitor
from 3 to ground. Stamping gives

```
        [ 1/(sL) + 1/R      -1/R      ]        rows and columns
   Y =  [                              ]       indexed by nodes 2, 3
        [    -1/R        1/R + sC     ]
```

and the determinant multiplies out to

```
 (1/(sL) + 1/R)(1/R + sC) - (1/R)(1/R)
   = 1/(sLR) + C/L + 1/R^2 + sC/R - 1/R^2
   = 1/(sLR) + C/L + sC/R              <- the 1/R^2 terms cancel
```

The band-pass has the inductor from 1 to 2, the capacitor from 2 to 3, and the resistor
from 3 to ground:
...
```
 ...
 (1/(sL) + sC)(sC + 1/R) - (-sC)(-sC)
   = 1/(sLR) + C/L + s^2C^2 + sC/R - s^2C^2
   = 1/(sLR) + C/L + sC/R              <- the s^2C^2 terms cancel
```

Two different matrices, three identical terms. Put them over $sLR$ and both read

$$\det Y(s) = \frac{s^2LC + sRC + 1}{sLR}.$$

The numerators are where the circuits part company. The source injects
$V_{in}/(sL)$ into row 2 and nothing into row 3, so Cramer's rule gives
$V_3 = -y_{32}\,(V_{in}/(sL))/\det Y$, and $y_{32}$ is the one entry that differs: $-1/R$
for the low-pass, $-sC$ for the band-pass. Cancelling against the $sLR$ leaves

$$H_{\text{low}}(s) = \frac{1}{s^2LC + sRC + 1}, \qquad
H_{\text{band}}(s) = \frac{sRC}{s^2LC + sRC + 1}.$$

Now the numbers. With $L = 0.1$ H, $C = 2.5\,\mu$F and $R = 100\,\Omega$ the denominator
is $2.5\times10^{-7}s^2 + 2.5\times10^{-4}s + 1$, or after dividing by $LC$,

```
 s^2 + (R/L) s + 1/(LC)  =  s^2 + 1000 s + 4.0e6

 roots:  s = (-1000 +/- sqrt(1e6 - 1.6e7))/2  =  -500 +/- j1936.49

 wn    = sqrt(1/(LC))  = 2000 rad/s = 318.31 Hz
 zeta  = (R/2) sqrt(C/L) = 50 * 0.005 = 0.25
 sigma = zeta * wn = 500 /s  ->  envelope time constant 1/500 = 2 ms
 wd    = wn sqrt(1 - zeta^2) = 1936.49 rad/s = 308.20 Hz
```

Those five numbers belong to the denominator, so they belong to both circuits, which is
why both transients ring at 308 Hz and die away in 2 ms. Everything the sweep showed as
different is in the numerator.

Here is the whole argument as twenty lines that run. The stamp is the module 6 stamp with
complex numbers in it, and the two wirings differ only in the element list:

```python
"""One stamp, two wirings: the same determinant, a different numerator."""
...
def admittance(kind, value, s):
    return {"R": 1.0 / value, "C": s * value, "L": 1.0 / (s * value)}[kind]


def stamp(elements, s, n=3):
    """The n x n admittance matrix, one element at a time. Node 0 is ground."""
    Y = [[0j] * n for _ in range(n)]
    for kind, a, b, value in elements:
        y = admittance(kind, value, s)
        if a:
            Y[a - 1][a - 1] += y
        if b:
            Y[b - 1][b - 1] += y
...
def det_and_gain(elements, s):
    """det of the two unknown rows, and V3 when a 1 V source holds node 1."""
    Y = stamp(elements, s)
    det = Y[1][1] * Y[2][2] - Y[1][2] * Y[2][1]
    rhs2, rhs3 = -Y[1][0], -Y[2][0]
    v3 = (Y[1][1] * rhs3 - Y[2][1] * rhs2) / det
    return det, v3


pole = complex(-500.0, 1936.4916731037085)
for name, net in (("low-pass ", low_pass), ("band-pass", band_pass)):
    print("%s  det Y(j2000) = %.6f uS   |det Y(pole)| = %.0e" %
          (name, det_and_gain(net, 2000j)[0].real * 1e6,
           abs(det_and_gain(net, pole)[0])))
for w in (200.0, 2000.0, 20000.0):
    print("w = %7.0f rad/s   low-pass |H| = %.7f   band-pass |H| = %.7f" %
          (w, abs(det_and_gain(low_pass, complex(0.0, w))[1]),
           abs(det_and_gain(band_pass, complex(0.0, w))[1])))
```

Both determinants print 25.000000 µS at $s = j2000$, and both collapse to a magnitude
below $10^{-19}$ at $s = -500 + j1936.49$ — zero, as far as double precision can tell,
which is what it means for that value of $s$ to be a natural frequency. The responses
that follow are $1.0088152$, $2.0000000$, $0.0100882$ for the low-pass and
$0.0504408$, $1.0000000$, $0.0504408$ for the band-pass. Two curves, one denominator.

## What the numerator is allowed to change

The $s$ in $H_{\text{band}}$ is a zero at the origin, and it is the whole difference. At
low frequency the denominator is near 1, so $|H_{\text{band}}| \approx \omega RC$: the
gain rises 20 dB per decade out of nothing at DC, which the printed $0.0504$ at
200 rad/s confirms against $200 \times 2.5\times10^{-4} = 0.05$. Well above resonance the
denominator is dominated by $s^2LC$, and the ratio becomes $R/(\omega L)$ — a fall of
20 dB per decade, which is the pair of poles giving $-40$ net of the zero's $+20$. At
20 000 rad/s that estimate is $100/2000 = 0.05$ against the printed $0.0504$.

At $\omega_n$ itself the reactances cancel: $s^2LC + 1 = 0$ leaves the denominator equal
to $sRC$, which is the numerator, so $|H_{\text{band}}| = 1$ exactly and the phase is
exactly zero. That holds for every damping ratio, not only this one. The low-pass has the
same denominator and a numerator of 1, so at the same frequency its magnitude is
$1/(\omega_n RC)$; and since $\omega_n RC = R\sqrt{C/L} = 2\zeta$, that number is
$1/(2\zeta) = 2$.

## The mistake people actually make

Look again at the band-pass skirts: 20 dB per decade below the centre, 20 dB per decade
above it, symmetric on a log axis. The natural reading is that one pole is taking care of
the low side and the other of the high side, the way a real high-pass in front of a real
low-pass would.

That reading is wrong here, and it is tempting because it is right somewhere else. A
wide-band filter built as an RC high-pass followed by an RC low-pass does have two real
poles, one per corner, and the picture is accurate. This circuit has $\zeta = 0.25$ and
its poles are the complex conjugate pair $-500 \pm j1936$. Neither of them is at
248 Hz and neither is at 408 Hz, and neither belongs to a side; they act together, over
the whole sweep. The skirts are symmetric because the *zero at the origin* subtracts a
constant 20 dB per decade from a response that was falling at 40 above resonance and
flat below it — not because the poles divided the work.

The test that separates the two cases is the $-3$ dB width. Here it is $R/L = 1000$ rad/s,
from 1561.55 to 2561.55 rad/s, or 248.53 Hz to 407.68 Hz, and the ratio of the centre
frequency to that width is $Q = 1/(2\zeta) = 2$. A cascade of two independent corners
cannot produce a $Q$ above $1/2$, so a measured $Q$ of 2 rules the picture out. Raise
$\zeta$ to 1 and the poles do become real and coincident; beyond that they separate into
two real corners, the two descriptions meet, and the mistake stops costing anything —
which is why it survives so long.

The second mistake is cheaper to make and cheaper to catch: writing impedances into $Y$.
Nodal analysis adds *currents* along a row, and current is admittance times voltage, so a
$1/(sC)$ where an $sC$ belongs turns a low-pass into a high-pass with a plausible-looking
plot and no error message anywhere.

## Where this stops holding

$\det Y(s)$ is the characteristic polynomial of the network, but it is not always the
denominator you finally see. If a numerator cofactor happens to share a root with it, the
factor cancels and that transfer function has one pole fewer than the circuit has natural
frequencies. The mode has not gone anywhere; it has become invisible from that particular
probe, and it can still be excited, still store energy and still saturate something. This
is why a cancelled pole is a fact about a measurement rather than about a circuit.

The other limits are the ones module 6 already named, arriving here with $s$ attached. A
voltage source floating between two unknown nodes fixes only their difference and needs a
supernode or the modified nodal analysis in `src/circuit.js`. A dependent source stamps a
term into a row belonging to nodes it does not touch, so $Y$ stops being symmetric and
the guarantee of a well-behaved answer goes with it. And nothing here is nonlinear: a
diode has no admittance, and a simulator handles it by linearising about a guess and
calling this machinery a few dozen times rather than once.

Finally, a practical limit. Nobody computes $\det Y(s)$ symbolically for a real netlist —
it is a determinant of a matrix with ten thousand rows. What a simulator does instead is
fix $s = j\omega$, which turns $Y$ into a matrix of ordinary complex numbers, and factor
it once per frequency point by the elimination of module 7. The cost of a sweep is one
solve per point, and the algebra never appears.

That solve is the lab, **The AC sweep, one complex solve at a time**. It is the
conductance-matrix function from module 6 with two lines changed — a complex array
instead of a real one, and `1j * w * value` for a capacitor — and its checks run the same
three components as both filters and compare each against the closed forms derived above.
If one piece of code reproduces a peak of 2 and a flat-topped 1 from the same denominator,
the claim of this module has been demonstrated rather than asserted. The derivation
**The determinant is the characteristic polynomial** walks the $2\times2$ stamp through
by hand first, and the build exercise **The same poles, a different probe** asks you to
move the probe on the schematic and watch the poles refuse to move with it.
''',
                },
            ],
            "build": {
